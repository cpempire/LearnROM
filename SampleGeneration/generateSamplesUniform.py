...
class GenerateSamplesUniform:
    ...
    def SubspaceSampling(self, nModes=0, nSamples=1):
        ...
        for n in range(nSamples):
            ...
            for i in range(nModes):
                sample += np.dot(self.samples[n], np.dot(self.HiFi.CovInv, self.U[:,i]))*self.U[:,i]
    ...
    def SubspaceProjection(self, nModes=0, sample=None):
        ...
        for i in range(nModes):
            sampleProjection += np.dot(sample, np.dot(self.HiFi.CovInv, self.U[:,i]))*self.U[:,i]
    ...
    def HessianSet(self, nTotalModes=64, nSamples=32):
        ...
        ## C^{-1} orthogonal basis
        CovMat = np.dot(np.dot(U_set.T, self.HiFi.CovInv), U_set)
        ...
        self.d = d
        # self.U = np.dot(self.HiFi.CovSqrt,np.array(self.U).T)
    ...
    def ActiveSubspace(self, nTotalModes=64, nSamples=0):
        ...
        d, U = la.eigh(GradientMat, self.HiFi.CovInv)
        sort_perm = np.abs(d).argsort()
        sort_perm = sort_perm[::-1]
        d = d[sort_perm]
        U = U[:,sort_perm]
