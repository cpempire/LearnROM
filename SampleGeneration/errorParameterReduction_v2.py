    ...
    for n in range(nSamples):
        ...
        if sampling.distribution is 'uniform':
            for i in range(nModes):
                sample += np.dot(test_samples[n], sampling.U[:,i])*sampling.U[:,i]
...
def ErrorParameterReductionV2(HiFi, samples, nSamples=1, sampling=None, nPRset=1, qoi=None, plot=False):
